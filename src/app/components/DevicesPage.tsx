                      ...
                      <div className="list-group list-group-flush">
                                ...
                                <div
                                  ...
                                  style={{
                                    ...
                                    backgroundColor: device.status === 'online'
                                      ? selectedDevice?.id === device.id ? '#ffffff20' : '#28a74520'
                                      : selectedDevice?.id === device.id ? '#ffffff20' : '#dc354520',
                                  }}
                                >
                                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                                    <rect x="4" y="2" width="16" height="20" rx="2" stroke={selectedDevice?.id === device.id ? 'white' : 'currentColor'} strokeWidth="2" />
                                    <path d="M12 18h.01" stroke={selectedDevice?.id === device.id ? 'white' : 'currentColor'} strokeWidth="2" />
                                  </svg>
                                </div>
                                <div>
                                  ...
                                  <div className="d-flex align-items-center">
                                    <span
                                      className={`badge me-2 ${device.status === 'online' ? 'bg-success' : 'bg-danger'}`}
                                      style={{
                                        backgroundColor: device.status === 'online'
                                          ? selectedDevice?.id === device.id ? '#28a745' : undefined
                                          : selectedDevice?.id === device.id ? '#dc3545' : undefined
                                      }}
                                    >
                                      {device.status}
                                    </span>
                                    <small className={selectedDevice?.id === device.id ? 'text-white-50' : 'text-muted'}>
                        ...
                        {devices.length === 0 && (
                          <div className="list-group-item text-center py-5">
                            ...
                            <small className="text-muted">Click "Add Device" to create one</small>
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="card-body p-4">
                        <div className="d-flex align-items-center justify-content-between mb-4">
                          <div>
                            ...
                            <span className={`badge ${selectedDevice.status === 'online' ? 'bg-success' : 'bg-danger'}`}>
                        ...
                        <div className="list-group">
                                ...
                                <div className="d-flex align-items-center flex-grow-1">
                                  ...
                                  >
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                                      ...
                                      <path d="M6 21v-2a4 4 0 014-4h4a4 4 0 014 4v2" stroke="#7c3aed" strokeWidth="2" />
                                    </svg>
                                  </div>
                                  <div className="flex-grow-1">
                                    ...
                                    <div className="d-flex align-items-center">
                                      <span className={`badge me-2 ${user.userRole === 'admin' ? 'bg-primary' : 'bg-secondary'}`}>
                                        {user.userRole}
                                      </span>
                                      <small className="text-muted">Config: {user.activeConfigurationId}</small>
                                    </div>
                                  </div>
                                </div>
                                <button
                          ...
                          {selectedDevice.users.length === 0 && (
                            <div className="list-group-item text-center py-5">
                              <p className="text-muted mb-0">No users assigned to this device</p>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="card border-0 shadow-sm">
                      <div className="card-body p-5 text-center">
                        <svg width="64" height="64" viewBox="0 0 24 24" fill="none" className="mb-3">
                          <rect x="4" y="2" width="16" height="20" rx="2" stroke="#ccc" strokeWidth="2" />
                          <path d="M12 18h.01" stroke="#ccc" strokeWidth="2" />
                        </svg>
                        <h5 className="text-muted">Select a device to view details</h5>
                        <p className="text-muted">Choose a device from the list to manage its users</p>
                      </div>
                    </div>
                  )}
                </div>
